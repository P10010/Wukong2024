...
void PBD::initializeFromFile(const std::string& filename)
{
    ...
    // The boat is simulated as a static rigid body (ie the vertex positions are not modified by the constraints)
...
T PBD::closestToPointInTriangle(const PBD::TV& q, const PBD::TV& p1, const PBD::TV& p2, const PBD::TV& p3, PBD::TV& closestPoint){
  ...
  //closest point on the plane is not inside the triangle -> choose the vertex p1
...
//Compute hash value given 3 discrete coordinates
...
/*Compute hash value associated to the 3D point
 * First, the continuous coordinates are turned to discrete ones in the range starting at
 * the corresponding minCoord and with l as the grid size
 */
...
/*
 * Initialize a hash table with the hash values associated to all the vertices of the simulated object (sail)
 */
...
/*
 * Use spatial hashing to find dynamic collision constraints
 */
...
//     T boxSize = 2 * h;
    ...
    for (int f = 0; f < faces.rows(); f++)
    {
        ...
        /*
         * Calculate the bounding box of the face, with the discrete coordinates that are used
            ...
            for (int j = minBox[1]; j <= maxBox[1]; j++)
                for (int k = minBox[2]; k <= maxBox[2]; k++)
                {
                    ...
                    for (int qIdx : hashTable[hashVal]) {
                        //necessary condition for intersection: x_i -> p_i passes through the plane containing the face
...
/*
 * Use triangle-triangle intersection tests from igl to generate collision constraints between the moving sail and the static boat
 */
    ...
    for (int i = minBox[0]; i <= maxBox[0]; i++)
      for (int j = minBox[1]; j <= maxBox[1]; j++)
        for (int k = minBox[2]; k <= maxBox[2]; k++) {
          ...
          for (int qIdx: hashTable[hashVal])
            /*
             * If any of the faces that qIdx belongs to intersect the boat face,
             * I add a constraint that a static constraint with parameters qIdx, the first intersection point, and the boat face normal
             */
...
/*
 * For all the points that need to be checked, perform a ray-mesh intersection test
 * to verify whether the ray x_i -> pi intersects or is inside the boat
 * Then perform further controls and decide whether to generate a static collision constraint
 */
void PBD::findStaticCollisions(std::vector<bool> toCheck){
  for (int i = 0; i < p.rows(); ++i) {
    if(toCheck[i]){
      ...
      if (igl::ray_mesh_intersect(xi, rayDir, boatV, boatF, hits)) {
        bool found = false;
        /*
         * If the number of intersections with the mesh is even, it means that the segment xi->pi
         * is not fully inside the boat
         */
        if (hits.size() % 2 == 0) {
          /*
           * Find the closest intersection point
           * If minHit.t<1, the segment xi->pi intersects the boat
           */
          auto minHit=hits[0];
          for (auto hit: hits) {
            if(hit.t<minHit.t)
          ...
          if (minHit.t < 1.f) {
            CollisionConstraintStatic constraint;
            constraint.pIdx = i;
            TV tmp = minHit.t * rayDir;
            constraint.q = xi + tmp;
            TV p1 = boatV.row(boatF(minHit.id, 0)), p2 = boatV.row(boatF(minHit.id, 1)), p3 = boatV.row(boatF(minHit.id, 2));
            constraint.n = ((p2 - p1).cross(p3 - p1)).normalized();
            collisionConstraintsStaticList.push_back(constraint);
            found = true;
          }
          ...
          // See this function for an attempt at finding the closest point on the surface
          // closestPointOnSurface(i);

          /* Workaround that produces better results but only in specific circumstances:
          * find the closest point on the mesh by going upwards
          * This helps in keeping eg a falling cloth above the boat, but is not a general solution
          */
          TV zAxis = {0,0,1};
...
/*
 * Generate static collision constraints using spatial hashing
 */
...
//  return;
   ...
   * among the faces that are close to pi
   */
...
//  std::vector<int> rayInsideMesh(p.rows(),-1);
  ...
  /* STRATEGY 3:
   * Use spatial hashing to find a list that are close to the boat and should be
   * checked for potential collisions
   */
      ...
      for (int i = minBox[0]; i <= maxBox[0]; i++)
        for (int j = minBox[1]; j <= maxBox[1]; j++)
          for (int k = minBox[2]; k <= maxBox[2]; k++)
          {
            int hashVal = hash(i, j, k, n);
            for (int qIdx : hashTable[hashVal]) {
              //Attempt to find the closest point on the surface by storing the face which is closest to it
//                TV closestPoint;
...
//    }
  ...
  /*
   * For all the points that need to be checked, perform a ray_mesh intersection test
   * to verify whether the ray x_i -> pi intersects or is inside the boat
   */
  findStaticCollisions(toCheck);
...
void PBD::generateCollisionConstraintsStatic(){
  ...
  if(useSpatialHashing)
    ...
    // Perform a ray/segment intersection test for all the points of the sail
    std::vector<bool> toCheck(p.rows(),false);
    findStaticCollisions(toCheck);
...
/*
 * Generate the collision constraints between dynamic colliding objects
 * In the current implementation, these are the self collisions of the boat with itself
 */
...
/*
 * Apply collision constraints with a static rigid body (the boat)
 */
...
/*
 * Apply the collision constraints between dynamic colliding objects
 * In the current implementation, these are the self collisions of the boat with itself
 */
void PBD::collisionConstraints()
{
    for (auto& constraint : collisionConstraintsList)
    {
        ...
        //The constraint function, and therefore whether it comes from below with respect to the triangle normal,
        //is determined when the constraint is generated
        int fromBelow;
        fromBelow=constraint.fromBelow;
        if (fromBelow * dist < (constraint.inBoat ? maxEdgeLength : h))
        {
            // The derivatives were calculated manually/analytically,
            // I have checked the calculations multiple times, so they should be correct
...
void initializeEdgeDataStructures(){
...
void PBD::edgeSelfCollisionConstraint(std::vector<std::vector<int>>& hashTable, const TV& minCoord, const TV& maxCoord,  int n, const T boxSize){
...
/*
 * Add to a hash table the hash values associated to all the vertices of the boat
 */
void PBD::hashBoatVertices(std::vector<std::vector<int>>& hashTable, T boxSize,
                       TV& minCoord)
{
  int n = hashTable.size();
...
/*
 * Failed attempt to use igl triangle triangle intersection test for self collision detection
 */
...
/*
 * Generate dynamic collision constraints between point i of the sail and the boat
 */
void PBD::generateBoatDynamicCollisionConstraints(int i){
  for (int f = 0; f < boatF.rows(); f++)
    {
        int t1 = boatF(f, 0), t2 = boatF(f, 1), t3 = boatF(f, 2);
        TV q = p.row(i);
...
void PBD::checkStaticCollision(int qIdx, int f, const TV& p1, const TV& p2, const TV& p3, std::vector<int>& rayInsideMesh, std::vector<CollisionConstraintStatic>& closestPoints){
  TV xi=currentV.row(qIdx);
  TV pi=p.row(qIdx);

  //necessary condition for intersection: x_i -> p_i passes through the plane containing the face
  //ie p_i being above the triangle and x_i below, or viceversa
  int qAbove=isAbove(pi, p1, p2, p3);
...
/*
 * Incorrect attempt at finding the closest point on the surface by shooting a ray in the reverse direction
 */
void PBD::closestPointOnSurface(int i){
  std::vector<igl::Hit> hits;
  TV xi = currentV.row(i);
  TV pi=p.row(i);
  TV rayDir=pi-xi;
